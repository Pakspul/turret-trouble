...
/* ══ Frontier ════════════════════════════════════════════════════════════
   ...
   low ground, hills and plateaus replaces the grid, and the game turns
   into a small real-time war: each side has a Commander that walks the
   map and builds, Factories that turn gold into a steady stream of units,
   and the same towers as Holdout for point defence. The enemy plays by
   the same rules. Nothing is won until the other side is annihilated.   */
...
export const MAP_W = 64, MAP_H = 40;
...
/** Sector s pays points like Holdout this many waves deep. */
...
/** Cells from the Commander within which it can build and repair. */
export const BUILD_REACH = 4;
/** Most construction orders one side may have waiting at once. */
export const SITE_CAP = 16;

/* ── economy ──────────────────────────────────────────────────────────────
   No mining: both sides are paid a steady income that rises with the
   clock, and a share of the price of everything they destroy.           */
export const START_GOLD = 700;
export const INCOME_BASE = 18;
export const INCOME_PER_MIN = 2;
/** Share of a unit's (or building's) price paid to whoever destroys it. */
export const SALVAGE = 0.2;

/* ── the Commander ────────────────────────────────────────────────────────
   Walks where you steer it, builds whatever is ordered within its reach,
   patches up your buildings when it has nothing to build, and fights.
   When it falls it goes up in a blast that hurts everything around it. */
export const COMMANDER = {
  hp: 9000, spd: 1.7, range: 3.4, dmg: 55, rate: 0.5, splash: 0.7, regen: 8,
  repair: 0.05, blast: 3.2, blastDmg: 2500, rad: 0.46
};

/* ── buildings ────────────────────────────────────────────────────────────
   `bt` is build time in Commander-seconds. A Factory is 3x3 and solid;
   the twelve cells along its sides (the ring, corners left out) take
   Helpers, which walkers pass straight through.                        */
export const FAB = { name: 'Factory', cost: 380, bt: 14, hp: 2600, color: '#8fb3c9' };
export const HELPER = { name: 'Helper', cost: 90, bt: 4, hp: 320, color: '#9fd0b0' };
/** Every further Factory costs this much more than the last. */
export const FAB_ESCALATION = 1.2;
/** Build speed each finished Helper adds to its Factory. */
export const HELPER_BOOST = 0.35;
/** Centres of two Factories stay this far apart, so rings never overlap. */
export const FAB_SPACING = 5;
/** Tower build time: seconds per gold of price. */
export const TOWER_BT = 1 / 30;
/** Tower health; grows with level. */
export const TOWER_HP = { gun: 420, rocket: 480, laser: 460, frost: 480, tesla: 520, portal: 560, factory: 520 };
export const TOWER_HP_LEVEL = 1.35;
/** Most Factory tiers; each is a paid upgrade that takes time. */
export const fabUpCost = tier => Math.round(650 * Math.pow(2, tier - 1) / 5) * 5;
export const fabUpTime = tier => 20 * Math.pow(1.3, tier - 1);
/** A unit from a tier-T Factory: power, price and build time. */
export const TIER_POWER = 1.7;
export const TIER_COST = 1.5;
export const TIER_TIME = 1.2;
/** Longest build sequence a Factory accepts. */
export const SEQ_MAX = 8;
/** A Factory can hold its units at the door and send them in groups. */
export const GROUPS = [1, 3, 6, 10];
/** Most units one side can have in the field; Factories wait past it. */
export const UNIT_CAP = 60;

/* ── units ────────────────────────────────────────────────────────────────
   `role` decides what a unit stops for:
     escort - fights anything hostile, buildings last
     raider - ignores troops and runs for the buildings, around your
              towers' fields of fire when there is another road
     siege  - prefers buildings, fights troops when nothing else is near
     air    - flies straight over water and cliffs at its target
   `aa` units can shoot flyers. `vsBase` multiplies damage on buildings.
   `bt` is build time in seconds at tier 1, before Helpers.             */
export const UNITS = {
  trooper: { name: 'Trooper', cost: 50,  bt: 4,  hp: 170, spd: 1.3, range: 2.3, dmg: 14,  rate: 0.55, armor: 0,    vsBase: 1,   role: 'escort', aa: true,  col: '#a8ecbb', ecol: '#ff8f8f', blurb: 'rifles · fights anything, hits air' },
  striker: { name: 'Striker', cost: 70,  bt: 4.5, hp: 105, spd: 2.4, range: 1.9, dmg: 26,  rate: 0.5,  armor: 0,    vsBase: 1.5, role: 'raider', aa: false, col: '#ffe08a', ecol: '#ffb36b', blurb: 'fast raider · skirts your towers' },
  breaker: { name: 'Breaker', cost: 180, bt: 10, hp: 540, spd: 0.8, range: 3.4, dmg: 100, rate: 1.8,  armor: 0.3,  vsBase: 2.5, role: 'siege',  aa: false, col: '#f4a259', ecol: '#e0607a', blurb: 'siege tank · splash, wrecks bases', splash: 0.9 },
  gunship: { name: 'Gunship', cost: 150, bt: 8,  hp: 230, spd: 1.9, range: 2.0, dmg: 22,  rate: 0.45, armor: 0.1,  vsBase: 1.2, role: 'air',    aa: true,  col: '#c9a7ff', ecol: '#ff9ad0', blurb: 'flyer · crosses water and cliffs', fly: true }
};
export const UNIT_ORDER = ['trooper', 'striker', 'breaker', 'gunship'];
export const unitPower = tier => Math.pow(TIER_POWER, tier - 1);
export const unitCost = (key, tier) => Math.round(UNITS[key].cost * Math.pow(TIER_COST, tier - 1) / 5) * 5;
export const unitTime = (key, tier) => UNITS[key].bt * Math.pow(TIER_TIME, tier - 1);

/* ── the enemy ────────────────────────────────────────────────────────────
   Same rules, same buildings, same units. Deeper sectors pay the enemy
   more, arm it better and let its Factories start a tier up.           */
export const aiIncome = s => 1 + 0.12 * (s - 1);
export const aiKit = s => Math.pow(1.08, s - 1);
export const aiStartTier = s => 1 + Math.floor((s - 1) / 8);
/** Cores for annihilating a sector, on top of everything else. */
export const sectorBounty = s => Math.round(18 * wavePoints(sectorDepth(s) + 20));
/** Cores per enemy unit destroyed, before tier and Data Siphon. */
export const killPoints = s => 0.02 * wavePoints(sectorDepth(s) + 10);
  ...
  },
  {
    ...
    nodes: [
      ...
      { id: 'depots',   name: 'Forward Depots',    max: 4, base: 1500, growth: 1.8, minWave: FRONTIER_UNLOCK_WAVE,
        step: '+1 Commander build reach',
        blurb: 'Your Commander builds and repairs from further away.',
        value: l => `${BUILD_REACH + l} cells of build reach` },
      { id: 'nanolathe', name: 'Nanolathe',        max: ENDLESS, base: 1100, growth: 1.55, minWave: FRONTIER_UNLOCK_WAVE,
        step: '+15% Commander build speed',
        blurb: 'Buildings go up faster, so the Commander spends less time standing still.',
        value: l => `+${15 * l}% build speed` },
      { id: 'armour',   name: 'Commander Armour',  max: ENDLESS, base: 1000, growth: 1.55, minWave: FRONTIER_UNLOCK_WAVE,
        step: '+12% Commander health',
        blurb: 'Your Commander can walk closer to the front and live.',
        value: l => `+${12 * l}% Commander health` }
    ]
  }
];
